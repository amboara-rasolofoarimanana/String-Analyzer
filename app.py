...
if onglet == "💡 Indications":
    ...
    # ░░░ 1. FICHIER ONDULEUR ░░░
    st.subheader("1️⃣ Fichier onduleur")
    ...
    st.subheader("2️⃣ Fichier caractéristiques des strings")
    st.markdown("Format attendu :")
    ...
    - `puissance unitaire` : puissance d’un panneau en **kWc**
    ...
    st.warning("""
    - 1/ Les fichiers doivent être au **format Excel ou CSV**  
    ...
    - 5/ Vérifier que tous les fichiers onduleurs couvrent des **périodes** qui coïncident entre elles
    ...
    # 3. Upload des fichiers d’onduleurs
    ...
    if st.button("Valider"):
     ...
     # Vérification des fichiers d’onduleurs
        ...
        # Vérification du fichier des caractéristiques
    ...
    with st.expander("ETUDE GLOBALE", expanded=False):
        ...
        if fichier_onduleur and fichier_carac and fichier_irradiance:
            ...
            # 7. Union des index puis réindexation pour aligner les deux séries
            ...
            if option_etude == "🔍 Données générales":
                ...
                # Création du graphique puissance
                ...
                # Réindexation sur les mêmes index que précédemment
    ...
    with st.expander("SUIVI TEMPOREL DE LA PUISSANCE", expanded=False):
        ...
        if fichier_onduleur:
            ...
            # Filtrage pour le jour sélectionné
        ...
        if mode_perf == "🔍 Étude par onduleur":
            ...
            if fichier_onduleur and fichier_carac:
                ...
                if not df_filtré.empty:
                    ...
                    # Énergie réelle par string (en kWh)
                    ...
                    # Création du DataFrame avec les index numériques
                    ...
                    moyenne_globale = df_resultats["ratio kWh/kWc"].mean()
                        ...
                        # 2. Sélection des strings sous la moyenne
                    ...
                    for idx, row in strings_sous_moyenne.iterrows():
                        ...
                        # 3. Récupération des caractéristiques du string
                        ...
                        #5. Récupération de leurs ratios
                    ...
                    # Agrégation mensuelle : somme de l'énergie par string par mois
                    ...
                    # Liste triée des strings disponibles
                    ...
                    with st.expander("📋 Details", expanded=False):
                        ...
                        tableau_croisé = df_affichage.pivot_table(
                            index=df_affichage["year_month"].dt.strftime("%Y-%m"),  # lignes = mois formatés
            ...
            # 1. Récupérer la période commune à tous les onduleurs
            dates_min = []
            dates_max = []
            ...
            # 3. Calcul des ratios filtrés par période pour chaque onduleur
            all_ratios = []

                # Boucle sur chaque couple (fichier onduleur, fichier caractéristiques)
            for idx_onduleur, (f_onduleur, f_carac) in enumerate(zip(st.session_state.fichiers_onduleurs, st.session_state.fichiers_caracteristiques), start=1):
                ...
                # Lecture et préparation du fichier de puissance
